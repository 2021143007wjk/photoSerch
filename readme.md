...
| **transformers** | StreetCLIP 모델을 로드하고, 이미지로부터 특징 벡터를 추출하기 위한 프로세서 활용에 사용되었다 |
...
신구대학교 정문, 본관, 동관 등 주요 경로를 따라 55분 가량의 1개의 영상을 촬영했다. 특히 COLMAP의 복원율을 높이기 위해 천천히 걷기, 다양한 각도 촬영, 왕복 촬영(Loop Closure) 전략을 적용했다. 해당 영상을 0.5초 간격으로 영상을 프레임 단위로 추출하여 총 6000장 이상의 학습용 이미지 데이터셋을 구축했다.
...
| 주요 설정 | 설명 |
|:----|:----|
| **sequential_matcher** | 시퀀스 매칭, 사진들이 동영상처럼 순서대로(Filename 순) 찍혔다는 것을 전제로 하여 매칭 속도와 정확도를 높이는 방식 |
...
신구대학교 캠퍼스 주요 구역에 대한 고밀도 3D 포인트 클라우드 데이터를 성공적으로 생성했다. StreetCLIP모델의 결과와, 결과를 웹과 연결하는 fastAPI도 정상 작동한다. 초기에 설계한 대로 gps없이 위치 예측에 성공하였다.
...
본 프로젝트는 별도의 비콘(Beacon)이나 하드웨어 설치 없이, 오직 영상 데이터와 AI 기술만으로 캠퍼스 내 위치 추정 시스템을 구현할 수 있음을 증명했다. 영상 데이터의 양을 늘려 더욱 완벽한 3d 맵을 제작하여 위치 예측의 정확도를 더욱 향상시킬 수 있을 것으로 보인다. 본 프로젝트에서는 야외에서 진행하였지만, 길 위주의 촬영이었기에 충분히 더 특색이 없는 지하에서의 학습도 가능할 것으로 보인다.
